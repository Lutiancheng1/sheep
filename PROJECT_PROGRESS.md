...
- [x] UI 视觉升级（仿羊了个羊）
  ...
  - [x] 整体配色：清新草绿背景
  - [x] 替换为SVG图标
    - [x] 设置图标:齿轮SVG (首页和游戏界面)
    - [x] 声音图标:音乐符号SVG
- [x] BGM 静音控制
  - [x] 创建 sound-on.svg 和 sound-off.svg 图标
  - [x] 在设置按钮下方添加声音控制按钮
  - [x] 实现点击切换静音/开启功能
...
- [x] 复活机制
  - [x] 广告复活流程
  - [x] 每日复活次数限制 (SystemConfig)

## 阶段 4.5：道具系统重构与广告接入（Milestone 3.2）

- [x] 道具系统逻辑重构
  - [x] 区分"库存道具"与"每日限制"
    - [x] 道具 (移出/撤销/洗牌): 改为永久库存制，不每日重置
    - [x] 复活: 保持每日限制，支持自定义重置时间
  - [x] 新用户默认赠送道具 (各2个)
  - [x] 后端 Entity 与 Service 重构 (ItemInventory)
- [x] 广告系统接入
  - [x] 前端 AdPlayer 组件开发
    - [x] 模拟视频播放与倒计时
    - [x] 放弃/领取奖励流程
    - [x] 防误触机制 (Stop Propagation)
  - [x] 广告配置管理 (SystemConfig)
    - [x] 视频地址、时长、每日限制可配
  - [x] 游戏内接入
    - [x] 失败弹窗接入广告复活
    - [x] 观看广告期间禁用游戏输入 (DISABLE_INPUT)
- [x] 关键 Bug 修复
  - [x] 修复广告"放弃"导致游戏重启问题
  - [x] 修复"撤回"道具随机摆放问题 (改为返回原位)
  - [x] 修复管理后台用户列表编辑道具功能
...
- [ ] 性能优化
  ...
  - [/] 加载速度优化
    - [x] 游戏场景加载优化
      - [x] GameScene 使用 `excludeData=true` 减少 API 数据传输量
      - [x] PhaserGame 事件驱动 Loading 显示（替换固定延迟）
      - [x] 添加 GAME_READY 事件通知机制
- [ ] 测试
  - [ ] 单元测试（游戏逻辑）
  - [ ] 集成测试
  - [ ] E2E 测试
